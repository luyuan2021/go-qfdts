...
func CheckMytools() {
	...
	//var tools = []string{"WQdump", "WQload", "mysqlpump"}
	for _, v := range tools {
...
func MyDumper(ip, port, userName, password string, backupdb []string, ignoredb []string, dumperthread string, dumploglevel int, dumpfile string) (err error) {
	...
	s1, _ := os.Getwd()
	cmd = "mydumper --user " + userName + " --password " + password + " -h " + ip + " -P " + port + " --regex " + ignoreRegx + " --triggers --events --routines --threads " + dumperthread + " --less-locking --build-empty-files --verbose " + strconv.Itoa(dumploglevel) + " --outputdir " + dumpfile + " --logfile " + s1 + "/mydump.log"
	//text := "WQdump命令如下：\n" + cmd
	PrintLog(cmd)
